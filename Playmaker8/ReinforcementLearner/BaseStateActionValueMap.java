...
public class BaseStateActionValueMap extends Object implements Serializable {
	    ...
	    public Action getBestAction(Percep state) {
	         if (stateActionValues.size() == 0)
	            return null;
	                   ...
	                   foundOne = true;
	                    ...
	                    if (!posAction.toString().equals("[NOOP NOTSET]")) {
	                        ...
	                        for (int j = 0; j < stateActionValues.size(); j++) {
	                            logfile.print("\n" + j + " "+ this.stateActionValues.get(j).toString(),1);
	                        }
	                        logfile.flush(1);
